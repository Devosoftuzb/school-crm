...
const getAllProduct = async () => {
  try {
    const data = await fetchData(
      `/student/${localStorage.getItem("school_id")}/archive-find`
    );
...
const getProduct = async (page) => {
  try {
    const data = await fetchData(
      `/student/${localStorage.getItem("school_id")}/archive/page?page=${page}`
    );
...
const archiveProduct = async () => {
  ...
  try {
    ...
    notification.success("O'quvchi arxivdan chiqarildi");
    getProduct(store.pagination);
